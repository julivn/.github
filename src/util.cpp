...
const char * const BITCOIN_CONF_FILENAME = "bitmonix.conf";
const char * const BITCOIN_PID_FILENAME = "bitmonixd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "bitmonix" is a composite category enabling all Bitmonix-related debug output
                if(ptrCategory->count(std::string("bitmonix"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bitmonix
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bitmonix
    // Mac: ~/Library/Application Support/Bitmonix
    // Unix: ~/.bitmonix
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bitmonix";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bitmonix";
#else
    // Unix
    return pathRet / ".bitmonix";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty bitmonix.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override bitmonix.conf
